...
match *match_descriptors(descriptor *a, int an, descriptor *b, int bn, int *mn)
{
    ...
    *mn = an;
    ...
    // printf("\t\tAux bn %d\n", bn);
    //  printf("\t\tAux bn-seen %ld\n/", sizeof(seen)/sizeof(int));
    ...
    for (i = 0; i < an; i++)
    {
        ...
        if (seen[m[i].bi])
        {
            ...
            i = count - 1;
            an -= 1; // Buat ngurangin jumlah yang dicek
        }
        else
        {
            seen[m[i].bi] = 1; //Untuk menandai bahwa sudah dicek
            count++;
        }
    }
    *mn = count;
...
point project_point(matrix H, point p)
{
    ...
    matrix Hc = matrix_mult_matrix(H, c);

    float x = Hc.data[0][0] / Hc.data[2][0];
    float y = Hc.data[1][0] / Hc.data[2][0];
    q = make_point(x, y);
    free_matrix(Hc);
    free_matrix(c);
    return q;
...
float point_distance(point p, point q)
{
    ...
    float dist_x = p.x - q.x;
    float dist_y = p.y - q.y;
...
int model_inliers(matrix H, match *m, int n, float thresh)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        float point_dist = point_distance(m[count].q, q_hat);
...
matrix RANSAC(match *m, int n, float thresh, int k, int cutoff)
{
    ...
    // Source: https://en.wikipedia.org/wiki/Random_sample_consensus#Algorithm
    for (int i = 0; i < k; i++)
    {
        randomize_matches(m, n);
        H = compute_homography(m, e);
        int n_inliers = model_inliers(H, m, n, thresh);
        if (n_inliers > best)
        {
            // printf("\tNumber of inliers: %d \n", n_inliers);
            best = n_inliers;

            Hb = compute_homography(m, n_inliers);
            // return immediately?
            if (best > cutoff)
            {
                free_matrix(H);
...
image combine_images(image a, image b, matrix H)
{
    ...
    for (k = 0; k < a.c; ++k)
    {
        for (j = topleft.y; j < botright.y; j++)
        {
            ...
            {
                point points = project_point(H, make_point(i, j));
                if (points.x >= 0 && points.y >= 0 && points.x < b.w && points.y < b.h)
                {
                    float value = bilinear_interpolate(b, points.x, points.y, k);
                    set_pixel(c, i - dx, j - dy, k, value);
                }
            }
        }
...
image cylindrical_project(image im, float f)
{
    ...
    float xc = im.w/2;
    float yc = im.h/2;
    float width = 2*f*atan2(xc, f)-1;
    image c = make_image(width, im.h, im.c);
    for (int k=0; k <im.c;k++){
        for (int y = -yc; y < c.h - im.h; y++)
        {
            for (int x = -width/2; x <= width/2; x++)
            {
                ...
                float X_hat = sinf(theta);
                float Y_hat = h;
                float Z_hat = cosf(theta);

                // Project to image plane
                float x_project = (f*X_hat/Z_hat)+xc;
                float y_project = (f*Y_hat/Z_hat)+yc;
                float value = bilinear_interpolate(im, x_project, y_project, k);
                set_pixel(c, x_project+width/2, y_project+yc,k, value);
            }
