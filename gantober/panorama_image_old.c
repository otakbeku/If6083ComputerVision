...
match *match_descriptors(descriptor *a, int an, descriptor *b, int bn, int *mn)
{
    ...
    *mn = an;
    ...
    for (j = 0; j < an; ++j)
    {
        ...
        // Anchornya dari A, jadi perulangan keduanya ada di B
        m[j].ai = j;
        m[j].p = a[j].p; // Assign points from A to j-index in m
        ...
        for (i = 0; i < bn; ++i)
        {
            ...
            if (distance < min_distance)
            {
                ...
                m[j].distance = min_distance = distance;
                m[j].bi = i;     //Assign index dari B yang paling kecil
                m[j].q = b[i].p; //Assign poin untuk M sesuai dengan point dari B
            }
        }
    }
...
point project_point(matrix H, point p)
{
    ...
    q.x = Hc.data[0][0] / Hc.data[2][0];
    q.y = Hc.data[1][0] / Hc.data[2][0];
    return q;
}
...
// image cylindrical_project(image im, float f)
// {
    // source: https://github.com/ybhan/Panoramic-Photo-Generation/blob/368e498b59d085d92dde5e155a3660b685ca1a9b/stitch.py#L162
//     //TODO: project image onto a cylinder
//     image c = copy_image(im);
//     return c;
// }